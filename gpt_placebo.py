#!/usr/bin/env python3
"""
GPT Dissent Scoring v6-PLACEBO - VOTE INFO PLACEBO CHECK
Same as v6 but WITHOUT the "ignore votes" instruction.
Compare v6 vs v6-placebo to see if vote info leaks into scoring.
"""
...
CHECKPOINT_FILE = f'{CACHE_DIR}/gpt_dissent_v6placebo_checkpoint.csv'
OUTPUT_FILE = f'{CACHE_DIR}/gpt_dissent_scores_v6_placebo.csv'

os.makedirs(CACHE_DIR, exist_ok=True)

print("="*70)
print("GPT DISSENT SCORING v6-PLACEBO - VOTE INFO CHECK")
print("="*70)
...
# ============================================================================
# DIRECTIONAL SCORING FUNCTION v6-PLACEBO
# ============================================================================

def score_dissent_direction(text, speaker):
    """GPT scoring v6-placebo: same as v6 but WITHOUT ignore-votes instruction"""
...
IMPORTANT:
- Score RELATIVE TO THE CHAIR'S PROPOSAL, not relative to current policy
- If the committee proposes a large rate cut and the speaker wants a smaller cut,
  that is HAWKISH (tighter than proposed), even though both favor cutting
- If the committee proposes holding rates and the speaker wants a small cut,
  that is DOVISH (easier than proposed)
- The question is always: does this speaker want TIGHTER or EASIER policy
  than what the Chair is proposing?
- Discussing regional weakness alone = +1 to +3, not strong dissent
- Most members agree (score 0) most of the time

HEDGING LANGUAGE:
- Pay close attention to "but", "however", "although", "yet", "that said", "nonetheless"
- When a speaker says "I support the proposal, but..." or "the economy is strong, however...",
  the statement AFTER the hedge often reveals their true preference
- Weight these qualifications heavily when scoring
...
print("\n[3] Testing v6-placebo prompt on 10 samples...")
print("="*70)
